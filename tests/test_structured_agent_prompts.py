...
def test_sentiment_prompt_states_constraint(monkeypatch):
    ...
    # The strings must clear the sentiment analyst's grounding hard-block
    # (>= _MIN_INFORMATIVE_CHARS of non-placeholder content); otherwise the LLM
    # is bypassed and no prompt is rendered to assert against.
    monkeypatch.setattr(
        sentiment, "fetch_stocktwits_messages",
        lambda *a, **k: "NVDA StockTwits: 42 bullish vs 8 bearish messages. " * 5,
    )
    monkeypatch.setattr(
        sentiment, "fetch_reddit_posts",
        lambda *a, **k: "r/wallstreetbets thread: NVDA earnings beat, 400 upvotes. " * 5,
    )
    monkeypatch.setattr(
        sentiment.get_news, "func",
        lambda *a, **k: "Nvidia announces record data-center revenue this quarter. " * 5,
        raising=False,
    )
