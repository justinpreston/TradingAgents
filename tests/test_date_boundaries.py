...
def test_load_ohlcv_requests_inclusive_end(monkeypatch, tmp_path):
    # This test pins the yfinance download date-boundary (#986). The fork
    # defaults core_stock_apis to polygon, which would route load_ohlcv through
    # the Polygon path and never call yf.download — so force the yfinance vendor.
    set_config({
