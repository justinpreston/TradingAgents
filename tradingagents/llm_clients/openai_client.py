...
# Provider-specific request headers (e.g., Copilot Chat API requires editor
# identification headers). Applied in get_llm() after the registry is consulted.
_PROVIDER_HEADERS = {
    "copilot": {
...
OPENAI_COMPATIBLE_PROVIDERS: dict[str, ProviderSpec] = {
    ...
    # Fork providers: GitHub Models + Copilot Chat, both authed with GITHUB_TOKEN.
    # Copilot's editor headers + GPT-5 Responses-API quirk are applied in get_llm().
    "github":     ProviderSpec(base_url="https://models.github.ai/inference"),
    "copilot":    ProviderSpec(base_url="https://api.githubcopilot.com"),
}
...
class OpenAIClient(BaseLLMClient):
    ...
    def get_llm(self) -> Any:
        ...
        if spec is not None:
            ...
            elif self.kwargs.get("api_key"):
                # An explicit api_key passed to the client (e.g. a corporate
                # proxy credential or a test placeholder) satisfies the auth
                # requirement; the passthrough loop below forwards it.
                pass
            elif api_key_env:
                raise ValueError(
        ...
        # Copilot's GPT-5 family only exposes /responses — chat/completions returns
        # unsupported_api_for_model for any gpt-5* model on api.githubcopilot.com.
        # (Native OpenAI's Responses API is enabled via spec.use_responses_api above.)
        if self.provider == "copilot" and self.model.lower().startswith("gpt-5"):
            llm_kwargs["use_responses_api"] = True
