...
# identification headers). ProviderSpec carries base_url/auth, but per-request
# headers are kept here and injected in get_llm(). GitHub Models and Copilot
# Chat are OpenAI-compatible endpoints we hit with a GITHUB_TOKEN.
_PROVIDER_HEADERS = {
    "copilot": {
...
OPENAI_COMPATIBLE_PROVIDERS: dict[str, ProviderSpec] = {
    ...
    # GitHub Models and Copilot Chat — OpenAI-compatible endpoints hit with a
    # GITHUB_TOKEN. Copilot also needs editor headers (see _PROVIDER_HEADERS).
...
class OpenAIClient(BaseLLMClient):
    ...
    def get_llm(self) -> Any:
        ...
        # Copilot's GPT-5 family only exposes /responses (the /chat/completions
        # endpoint returns unsupported_api_for_model for any gpt-5* model on
        # api.githubcopilot.com). Native OpenAI already gets the Responses API
        # from its registry spec above.
